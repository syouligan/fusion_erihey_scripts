...
sampleFiles <- list.files(path=samplePath, pattern= "unique\\.genes\\.results$", recursive=TRUE, full.names=TRUE)
...
if (length(ERCCFilter) != 0) {
  ...
  write.csv(RSEM_ERCC_FPKM, file=paste0(outPath, "RSEM_ERCC_FPKM_unique.csv", sep=""))
  write.csv(RSEM_ERCC_count, file=paste0(outPath, "RSEM_ERCC_count_unique.csv", sep=""))
  write.csv(RSEM_all_count, file=paste0(outPath, "RSEM_all_count_unique.csv", sep=""))
  write.csv(RSEM_FPKM, file=paste0(outPath, "RSEM_FPKM_unique.csv", sep=""))
  write.csv(RSEM_TPM, file=paste0(outPath, "RSEM_TPM_unique.csv", sep=""))
  write.csv(RSEM_count, file=paste0(outPath, "RSEM_count_unique.csv", sep=""))
} else {
  print("No ERCCs")
  write.csv(RSEM_FPKM, file=paste0(outPath, "RSEM_FPKM_unique.csv", sep=""))
  write.csv(RSEM_TPM, file=paste0(outPath, "RSEM_TPM_unique.csv", sep=""))
